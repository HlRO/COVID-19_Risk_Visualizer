...
def Analyse(name):
    # Load database.
    COVID_STATS_PATH = 'data/world/total_cases.csv'
    update_time = time.localtime(os.path.getmtime(COVID_STATS_PATH))
    cases = pd.read_csv(COVID_STATS_PATH)
    populations = pd.read_csv('data/world/population.csv')
    beds = pd.read_csv('data/world/beds.csv')
